...
static int fsread(const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	char *out = NULL; 
	printf("VOLANY FSREAD SIZE: %d, OFFSET: %d\n", size,offset);
	...
	if(out)
	{
		...
		printf("(FSREAD) VRACIAM DLZKU %d \n", dlzka);
		printf("IDEM PISAT DO BUFFRA VELKOSTI, DLZKA:%d a offset: %d\n", dlzka,offset);
		
		//memcpy(buffer, out + offset, dlzka + 1);
		int real_size = strlen(out);
		if(offset+size<=real_size) {
			memcpy(buffer, out + offset, size);
			printf("(FSREAD)PRED RETURN \n");
			free(out);
			return size;
		}
		else {
			int blocksize = real_size - offset;
			memcpy(buffer, out + offset, blocksize);
			printf("(FSREAD)PRED RETURN \n");
			free(out);
			return blocksize;
		}
